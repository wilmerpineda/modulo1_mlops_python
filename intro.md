# 📘 Módulo 1: Fundamentos de MLOps y Gestión del Ciclo de Vida

## 👋 Bienvenido(a)

Bienvenido(a) al primer módulo del curso **MLOps en Python**.

En este espacio comenzarás a construir una de las competencias más importantes en la práctica moderna de la ciencia de datos:
👉 **pasar de modelos experimentales a sistemas reproducibles, versionados y desplegables.**

A lo largo de este módulo, no solo aprenderás herramientas, sino que desarrollarás una forma de pensar el ciclo de vida completo de un modelo de Machine Learning.

---

## 🎯 Propósito del módulo

El objetivo principal de este módulo es que comprendas y apliques los fundamentos de **MLOps (Machine Learning Operations)**, integrando buenas prácticas de:

* ingeniería de software,
* ciencia de datos,
* y DevOps,

para construir soluciones que sean:

* reproducibles,
* trazables,
* colaborativas,
* y escalables.

---

## 🧠 ¿Por qué MLOps?

En muchos proyectos de ciencia de datos, el proceso termina en un notebook con buenos resultados. Sin embargo, en entornos reales esto no es suficiente.

Un modelo en producción requiere:

* control de versiones del código y los datos;
* gestión de dependencias y entornos;
* seguimiento de experimentos;
* reproducibilidad de resultados;
* colaboración entre múltiples actores.

👉 Aquí es donde entra MLOps.

Este módulo te permitirá cerrar la brecha entre el prototipo y el sistema.

---

## 🧩 Enfoque del Jupyter Book

Este material está diseñado para que aprendas de forma:

* **progresiva** → desde conceptos hasta implementación;
* **aplicada** → con ejemplos reales;
* **reproducible** → todo lo que ves, lo puedes ejecutar;
* **integrada** → cada tema se conecta con el siguiente.

Encontrarás dos tipos de contenidos:

### 📄 Secciones conceptuales

Explican los fundamentos teóricos y las buenas prácticas.

### 💻 Notebooks prácticos

Permiten ejecutar código, explorar herramientas y construir piezas del flujo MLOps.

---

## 📊 Caso de estudio transversal

Durante todo el módulo trabajaremos sobre un caso aplicado:

> **Predicción de Open Rate en notificaciones digitales**

Este caso será el hilo conductor para:

* construir datasets simulados;
* entrenar modelos de clasificación;
* versionar datos y código;
* registrar experimentos;
* y estructurar un proyecto reproducible.

👉 Este mismo caso será la base de la **actividad integradora del módulo**.

---

## 🛠️ Herramientas que utilizarás

A lo largo del módulo trabajarás con herramientas clave del ecosistema MLOps:

* **Git & GitHub** → control de versiones y colaboración
* **Poetry** → gestión de entornos y dependencias
* **DVC (Data Version Control)** → versionamiento de datos y pipelines
* **MLflow** → seguimiento de experimentos

El objetivo no es solo usarlas, sino entender **cuándo, por qué y cómo integrarlas**.

---

## 🧪 Metodología de trabajo

Este módulo está diseñado bajo un enfoque activo:

1. Lectura guiada del material
2. Ejecución de notebooks
3. Exploración y modificación del código
4. Construcción progresiva de un flujo MLOps
5. Aplicación en una actividad integradora en equipo

---

## ⚠️ Recomendaciones importantes

Para aprovechar al máximo este módulo:

* Ejecuta todos los notebooks (no solo los leas).
* Modifica el código y observa qué ocurre.
* Presta atención a la estructura de los proyectos.
* No memorices comandos: **comprende el flujo**.
* Trabaja desde el inicio con buenas prácticas.

---

## 🚀 Resultado esperado

Al finalizar este módulo, deberías ser capaz de:

* estructurar un proyecto de Machine Learning de forma profesional;
* gestionar entornos reproducibles;
* versionar datos y pipelines;
* registrar y comparar experimentos;
* trabajar colaborativamente con Git y GitHub.

---

## 🔗 ¿Cómo avanzar en el módulo?

Te recomendamos seguir el orden propuesto en el contenido:

1. Comprender el panorama de MLOps
2. Entender el ciclo de vida del modelo
3. Dominar el uso de Git y GitHub
4. Gestionar entornos con Poetry
5. Versionar datos con DVC
6. Registrar experimentos con MLflow
7. Integrar todo en un flujo completo

---

## 🎓 Cierre

Este módulo marca un punto de transición importante:

👉 de escribir código que funciona
👉 a construir sistemas que pueden mantenerse, escalar y colaborar

Bienvenido(a) a MLOps.

---
